<div class="span6">
  <div class="panel_block">
    <h2>概况</h2>
      <table class="table" ><thead><tr><th>Bug总数: <?=$stat[0]+$stat[1]?></th><th>未修复:  <?=$stat[0]?></th><th>已解决: <?=$stat[1]?>  </th></tr></thead></table>
  </div>
  <div class="panel_block">
    <h2>待办理</h2>
    <table class="table table-hover" >
      ...
      foreach($mybugs as $b){?>
        <tr>
          <td width="60%" ><a href="?/bugtrace/view/<?=$b['id']?>/" ><?=$b['title']?></a></td>
          <td width="15%" ><div class="cc cc<?=$b['priority']?>" ><?=$prio[$b['priority']]?></div></td>
          <td><?=$data['users'][$b['doer']]?></td>
        </tr>
        <?}?>
      </tbody>
    </table>
